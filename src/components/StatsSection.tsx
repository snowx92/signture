...
import { Users, BookOpen, Award, Globe, TrendingUp, Star, Target, Zap } from 'lucide-react';
...
const StatsSection = () => {
  ...
  const CounterAnimation = ({ number, suffix, isVisible }: { number: number; suffix: string; isVisible: boolean }) => {
    ...
    return (
      <span className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">
        {isClient ? formatNumber(count) : formatNumber(number)}{suffix}
      </span>
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute top-1/2 right-0 w-80 h-80 bg-gradient-to-br from-emerald-100/30 to-teal-100/30 rounded-full blur-3xl transform translate-x-1/2"></div>
        <div className="absolute bottom-0 left-1/3 w-72 h-72 bg-gradient-to-br from-orange-100/30 to-pink-100/30 rounded-full blur-3xl transform translate-y-1/2"></div>
        ...
        <div className="absolute top-1/4 left-1/4 opacity-5">
          <Star className="w-8 h-8 text-blue-500 animate-bounce" style={{ animationDelay: '0s', animationDuration: '3s' }} />
        </div>
        <div className="absolute top-1/3 right-1/4 opacity-5">
          <Target className="w-6 h-6 text-purple-500 animate-bounce" style={{ animationDelay: '1s', animationDuration: '4s' }} />
        </div>
        <div className="absolute bottom-1/3 left-1/2 opacity-5">
          <Zap className="w-7 h-7 text-emerald-500 animate-bounce" style={{ animationDelay: '2s', animationDuration: '5s' }} />
        </div>
        <div className="absolute bottom-1/4 right-1/3 opacity-5">
          <TrendingUp className="w-8 h-8 text-orange-500 animate-bounce" style={{ animationDelay: '0.5s', animationDuration: '3.5s' }} />
        </div>
      </div>
          ...
          {stats.map((stat, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`absolute top-0 right-0 w-24 h-24 ${stat.bgColor} rounded-full blur-2xl transform translate-x-1/2 -translate-y-1/2 group-hover:scale-150 transition-transform duration-500`}></div>
                
                {/* Icon container */}
                <div className="relative mb-6">
                  <div className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-r ${stat.color} shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}>
        ...
        <div className="text-center">
          <div className="inline-flex items-center justify-center space-x-8 p-6 bg-white/60 backdrop-blur-lg border border-gray-200/50 rounded-2xl shadow-lg">
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="text-sm font-medium text-gray-700">
                {direction === 'rtl' ? 'معتمد في الصناعة' : 'Industry Certified'}
              </span>
            </div>
          </div>
        </div>
