...
import { Award, BookOpen, Globe, Handshake, Users } from "lucide-react";
...
const PartnersSection = () => {
  const partners = [
    ...
    },
    {
      ...
      description:
        "Leading Arab educational exceعحيشفثيllence and accreditation organization",
      bgColor: "bg-green-50",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          {partners.map((partner) => (
            ...
            >
              <div className="p-8 md:p-10">
                ...
                <div className="text-center space-y-4">
                  <h3 className="text-2xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors duration-300">
                    <Message
                      id={`partners.name.${partner.id}`}
                      fallback={partner.name}
                    />
                  </h3>

                  <div
                    className={`inline-block px-4 py-2 bg-gradient-to-r ${partner.color} text-white rounded-full text-sm font-semibold shadow-lg`}
                  >
                    <Message
                      id={`partners.type.${partner.id}`}
                      fallback={partner.type}
                    />
                  </div>

                  <p className="text-gray-700 leading-relaxed text-base mt-4">
                    <Message
                      id={`partners.desc.${partner.id}`}
              ...
              <div
                className={`h-3 bg-gradient-to-r ${partner.color} transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-center`}
              ></div>
            </div>
          ))}
        </div>
